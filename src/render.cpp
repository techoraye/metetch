...
void renderDisplay(const Info& inf) {
    ...
    if (cfg.show_disk && inf.disk != "Unknown") {
        printProgressRow("Disk Usage", inf.mem_used, inf.mem_total, colors::COLOR_DISK);
    }
    ...
    if (inf.bat != "N/A" && !inf.bat.empty()) {
        printRow("Battery Status", inf.bat, colors::TEXT_VALUE);
    }
    ...
    printf("\n  %s⚡ Thanks you for using metetch | Made by Techoraye :) %s\n\n",
